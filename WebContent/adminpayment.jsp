...
<%@taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%><meta
...
<body>
	...
	<c:choose>
		<c:when test="${requestScope.listpayment!=null}">
			<c:set var="listPayment" value="${requestScope.listpayment}" />
		</c:when>
		<c:otherwise>
			<c:set var="listPayment" value="${DAOPayment().getList()}" />
		</c:otherwise>
	</c:choose>
								...
								<div class="row">
											...
											<div class="row">
												<div class="col-lg-4">
													<div class="form-group">
														<form action="PaymentController?actionManage=findWithName"
															method="post">
															<div class="input-bar-item">

																<div class="input-group">
																	<input name="nameSearch" class="form-control"
																		placeholder="Tìm theo tài khoản..."> <span
									...
									<div class="col-lg-2 col-md-2 col-sm-2 col-xs-12">
										<ul class="breadcome-menu">
											<li><span class="bread-blod">Quản lí đơn hàng</span></li>
										</ul>
									</div>
								</div>
							</div>
						...
						<table class="table table-striped table-bordered">
							...
							<tbody>
								...
								<c:forEach items="${listPayment }" var="p">
									<tr>
										<td>${p.getId()}</td>
										<td>${DAOUser().nameUser(p.getIdUser())}</td>
										<td>${p.getCount()}</td>
										<td>${p.getTotal()}</td>
										<td>${p.getTime()}</td>
										<td>${p.getAdress()}</td>
										...
										<td><a data-toggle="modal"
											data-target="#myModal${p.getId()}"><button
													style="margin: auto;" type="button" class="btn btn-info">
													<i class="fa fa-pencil"></i><span>Sửa</span>
												</button></a> <a
											href="PaymentController?actionManage=done&idOfPayment=${p.getId()}"><button
													style="margin: auto;" type="button" class="btn btn-success">
													<i class="fa fa-pencil"></i><span>Thanh toán</span>
												</button></a> <a
											href="PaymentController?actionManage=deletePayment&idOfPayment=${p.getId()}"><button
									...
									<div class="modal fade" id="myModal${p.getId()}" role="dialog">
										<form
											action="PaymentController?actionManage=alterPayment&idOfPayment=${p.getId()}"
											method="post">
												...
												<div class="modal-content">
													...
													<div class="modal-body">
														<div class="form-group row">
															...
															<div class="col-sm-9">
																<input class="form-control" value="${p.getId()}"
																	disabled="disabled">
															</div>
														</div>
														<div class="form-group row">
															...
															<div class="col-sm-9">
																<input class="form-control" name="alterIdOfUser"
																	value="${DAOUser().nameUser(p.getIdUser())}">
															</div>
														</div>
														<div class="form-group row">
															<label class="col-sm-3 col-form-label">Total</label>
															<div class="col-sm-9">
																<input class="form-control" name="alterTotal"
																	value="${p.getTotal()}">
															</div>
														</div>
														<div class="form-group row">
															...
															<div class="col-sm-9">
																<input class="form-control" name="alterCount"
																	value="${p.getCount()}">
															</div>
														</div>
														<div class="form-group row">
															...
															<div class="col-sm-9">
																...
																</select> <select id="month" name="alterMonth"
																	onchange="definedays()">
																	<c:forEach begin="1" end="12" var="j">
																		<option value="${j}">${j}</option>
																	</c:forEach>

																</select> <select id="year" name="alterYear"
																	onchange="definedays()">
														...
														<div class="form-group row">
															...
															<div class="col-sm-9">
																<input class="form-control" name="alterAdress"
																	value="${p.getAdress()}">
															</div>
														</div>
														<div class="form-group row">
															<label class="col-sm-3 col-form-label">Đã thanh toán:</label>
															<div class="col-sm-9">
																<div class="form-check">
																	<input class="form-check-input position-static"
																		type="checkbox" name="isDone" value="option1">
																</div>
															</div>
														</div>
													</div>
